...
typedef enum DECODE_VERIFY_SERVER {
    DECODER_SERVER_INIT         = 0,
    DECODER_SERVER_DESTORY      = 1,
    DECODER_SERVER_FIND_STATUS  = 2,
}eDecodeVerifyServer;
...
typedef struct DecVcuConfig
{
    unsigned int      bVcuAutoMode;
    unsigned int      nFrameNumInGroup;
    unsigned int      bEnableVcu ;
}DecVcuConfig;

typedef enum SCALEDOENMODE {
    SCALE_MODE_CLOSE = 0,
    SCALE_MODE_ONLINE_FIXRATIO,
    SCALE_MODE_ONLINE_RANDOM,
}SaleMode;
...
typedef struct VCONFIG
{
    ...
    int bAdapteDropFrame;

    struct ScMemOpsS *memops;
    eControlAfbcMode  eCtlAfbcMode;
    eControlAfbcMode  nAfbcSecMode;
...
int SetReleasePicture(VideoDecoder* pDecoder, VideoPicture* pPicture);


int EmptyPictureNum(VideoDecoder* pDecoder, int nStreamIndex);
int ReleasePictureNum(VideoDecoder* pDecoder, int nStreamIndex);
int DecoderServerCommand(eDecodeVerifyServer serverid, void *parameter);
